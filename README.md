...
- Отдельно от этого: **отзыв о сервисе** (бот спрашивает один раз после третьего хода) удаление
  переживает и остаётся привязанным к `chat_id`. Это добровольное показание о продукте, а не
  часть разговора; отзыв, который нельзя возвести к живому аккаунту, не работает как
  доказательство, что его не сочинил оператор. Текст `/privacy` говорит об этом прямо.
