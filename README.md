...
у вас в человекочитаемых `.md`-файлах. Голос или текст, из Telegram.
...
> оператора) — **не присылайте пароли и то, что нельзя показывать третьим лицам**. Нужна полная
> приватность — [разверните свой инстанс](#быстрый-старт), данные не покинут ваш сервер.
...
> кризиса, риска для жизни, насилия или психического расстройства обратитесь к живому
> специалисту или в службу экстренной помощи. Используя проект, вы принимаете этот риск на себя.
...
Честно: дело не в том, что модель умнее — мозгом AICOACH можно поставить хоть тот же Claude или
GPT. Разница в **обвязке вокруг модели**. Голый чат каждый раз даёт тебе гениального незнакомца;
AICOACH даёт метод и память.
...
Итог: платный чат — универсальный инструмент. AICOACH — специализированный контур для регулярной
работы над собой: с памятью, которой ты владеешь, и рамкой, которая не поддакивает.

## Архитектура

Не жёсткий пайплайн, а «голый мозг» со скиллом и инструментами к своей памяти — модель сама
решает, когда вспомнить и когда записать:
...
Первое сообщение боту вернёт ваш `chat_id` — впишите его в `ALLOWED_CHAT_ID` и перезапустите
(доступ только вам). Прод 24/7 — через systemd, см. [deploy/DEPLOY.md](deploy/DEPLOY.md).

## Приватность

- Профиль и переписка — в `tenants/` (в `.gitignore`, наружу не уходят).
- Наружу идёт только запрос к выбранной вами модели. Хотите, чтобы **ничего** не покидало
  машину, — подключите локальную модель (Ollama) в `.env`.
...
даёт и чего честно не даёт:

- **`tenant_id` = соль + hash(chat_id)** — на диске каталог с профилем не подписан вашим
...
реализованная возможность. Для настоящей приватности — разверните свой приватный инстанс.
