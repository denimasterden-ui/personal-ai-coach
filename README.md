...
Напиши ему, что сейчас занимает или тревожит, — и получишь разбор. `/start` — что это и как
устроено, `/memory` — что бот о тебе понял, `/export` — забрать свою память, `/delete_my_data` —
стереть всё.
